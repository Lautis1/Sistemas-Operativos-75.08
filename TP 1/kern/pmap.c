...
page_init(void)
{
	...
	// free pages!
	size_t i;
	uint32_t page_iophysmem = PGNUM(IOPHYSMEM);
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Asserts para ver si todo esta bien (dicho por dato)
	assert(va % PGSIZE == 0);
	assert(pa % PGSIZE == 0);
	assert(size % PGSIZE == 0);
	assert(perm < (1 << PTXSHIFT));
	while (size > 0) {
// Se busco usar if defined(TP1_PSE) pero el test de large pages necesita un ifndef.
#ifndef TP1_PSE
#else
		// alineada a 4 MiB fisica y virtual
...
{
	//*pgdir: puntero a un directorio en memoria
	//*pp: puntero a un pageInfo que corresponde con la pagina
	// fisica que se quiere insertar
	//*va: direccion virtual en la que se va a mapear la pag fisica
	// perm: flags ¿¿?


	// CREATE en 1 para decirle a pgdir_wal() que cree una nueva
	// page table vacia si no existe
	pte_t *pte = pgdir_walk(pgdir, va, 1);

	// Si no se pudo alocar la pte devolver -E_NO_MEM
	if (!pte)
		return -E_NO_MEM;


	// pp->pp_ref debe ser incrementado si se dio la insercion
	pp->pp_ref++;

	// Remover cualquier pagina fisica que ya este mapeada en va
	page_remove(pgdir, va);

	// Setear los flags de permisos a perm|PTE_P
...
{
	// Hay que devolver la pag fisica donde se aloja VA
	...
	*pte_store = page_table_entry;
	...
	// Usar pa2page y PTE_ADDR
	// pa2page recibe un puntero a PageInfo y devuelve physaddr_t
	// PTE_ADDR recibe un physaddr_t
...
check_kern_pgdir(void)
{
	...
	// cprintf("%p\n", *pgdir_walk(pgdir, (const void *)UPAGES, 0));
	for (i = 0; i < n; i += PGSIZE) {
		// prints para ver q valores devuelven las funciones
	...
	for (i = 0; i < NPDENTRIES; i++) {
		...
		default:
			if (i >= PDX(KERNBASE)) {
				assert(pgdir[i] & PTE_P);
				assert(pgdir[i] &
				       PTE_W);  // Para que pase esta faltaba PTE_W en pgdirwalk.
			} else
				assert(pgdir[i] == 0);
			break;
		}
